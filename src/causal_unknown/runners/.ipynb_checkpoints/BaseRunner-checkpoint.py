...
class BaseRunner(object):
    def parse_runner_args(parser):
        ...
        parser.add_argument('--metrics', default='hit@1,hit@5', type=str,
    ...
    def _build_optimizer(self, model):
        ...
        elif self.opt_name == 'rnsprop':
            self.optimizer = opt.RMSprop(model.parameters(), lr=self.lr)
        else:
    ...
    def predict(self, model, data):
        ...
        for i, batchData in enumerate(dataLoader):
            ...
            for key, pred, lab in zip(user, prediction, label):
                p[key].append(pred)
                gt[key].append(lab)
        pbar.close()
    ...
    def evaluate(self, model, data):
        p, gt = self.predict(model, data)
        return evaluator.evaluate(p, gt, self.metrics)
